...
# %%
def set_psd(ws, psd, mgd_coef=None, scat_species_a=0.02, scat_species_b=2):
    if psd == PSD.D14:
        ws.psdDelanoeEtAl14(
            t_max=275,
            t_min=180,
...
def get_psd(fwc: np.array = None, t: np.array = None, psd=None, psd_size_grid=np.logspace(-6, -2, 100), mgd_coef=None, scat_species_a=0.02, scat_species_b=2,):
    if len(fwc) != len(t):
    ...
    set_psd(ws, psd, mgd_coef=mgd_coef, scat_species_a=scat_species_a, scat_species_b=scat_species_b)
