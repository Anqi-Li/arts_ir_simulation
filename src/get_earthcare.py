...
def prepare_arts(orbit_frame):
    ...
    # % filter bad pixels, low clouds, cleary sky
    ...
    ds = (
        ...
        .assign(
            dBZ=da_dBZ_height,
            **{v: (da_dBZ_height.dims, ds_xmet_height[v].__array__()) for v in vars},
        )
...
if __name__ == "__main__":
    ...
    orbit_frame = "01162E"
    ds = prepare_arts(orbit_frame)

    # %% save ds to netcdf file
    path_save = "../data/earthcare/xy_data/"
    ds.reset_index("nray").set_xindex("time").to_netcdf(path_save + f"xy_{orbit_frame}.nc")
