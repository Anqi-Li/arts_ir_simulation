...
psd_data = psd_F05(d_meq, t=t, iwc=iwc)  # m^-4

# test consistency
iwc_from_psd_integral_dmeq = np.trapezoid(np.pi * rho / 6 * d_meq**3 * psd_data, d_meq)  # kg/m^3
...
iwc_from_psd_integral_dmax = np.trapezoid(a * d_max**b * psd_data * ddmeq_ddmax, d_max)  # kg/m^3
...
for psd in [PSD.D14, PSD.F07T, PSD.MDG]:
    ...
    ax1.loglog(d_meq, psd_data.squeeze(), marker=".", markersize=1, linewidth=0.5, label=psd)
    ...
    x = d_meq / d0star
    ax2.semilogy(x, psd_data_norm.squeeze(), marker=".", markersize=1, linewidth=0.5, label=psd)
    print(psd)
    iwc_from_psd_integral = np.trapezoid(np.pi*rho/6 * d_meq**3 * psd_data, d_meq)
    print(
        f"IWC from psd integration over dmeq: {iwc_from_psd_integral:.3e} kg/m^3"
    )
    ...
    print(
        f"IWC from psd integration over dmax: {iwc_from_psd_integral:.3e} kg/m^3"
    )
    print('ratio to given IWC:', iwc_from_psd_integral/iwc)

    ax3.plot(d_meq, psd_data.squeeze()*d_meq**6, marker=".", markersize=1, linewidth=0.5, label=psd)

# ACMCAP for both plots
psd_data = psd_F05(d_meq, t=t, iwc=iwc)  # m^-4
ax1.loglog(d_meq, psd_data, marker=".", markersize=1, linewidth=0.5, label="ACM_CAP")

x = d_meq / get_d0star_from_n0star_iwc(
    n0star=get_n0star_from_temperature(t=t), iwc=iwc
)
ax2.semilogy(x, psd_F05_norm(x), marker=".", markersize=1, linewidth=0.5, label="ACM_CAP")
print("ACM_CAP")
iwc_from_psd_integral = np.trapezoid(np.pi*rho/6 * d_meq**3 * psd_data, d_meq)
print(
    f"IWC from psd integration over dmeq: {iwc_from_psd_integral:.3e} kg/m^3"
...
ax3.plot(d_meq, psd_data.squeeze()*d_meq**6, marker=".", markersize=1, linewidth=0.5, label="ACM_CAP")
...
for i, n0star in enumerate(n0star_values):
    ...
    plt.loglog(d_meq, psd_cap, label=f'T={t_c[i]} °C, N0*={n0star:.2e}, D0*={d0star*1e3:.2f} mm')
